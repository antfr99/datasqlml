...
unwatched_top_1000 = top_1000[~top_1000["Title"].str.lower().isin(watched_titles)].copy()

# --- Find your favorite genres from Personal Ratings ---
fav_genres = (
    Personal_Ratings[Personal_Ratings["Personal Ratings"] >= 7]["Genre"]
    .dropna()
    .str.split(",").explode().str.strip()
    .value_counts().head(5).index
)

# --- Add a simple predicted_score column ---
def score_movie(row):
    score = 0
    if row.get("IMDB_Rating", 0) >= 7.5:  # strong IMDb rating
        score += 2
    if any(g in str(row.get("Genre", "")) for g in fav_genres):  # genre match
        score += 1
    if row.get("Meta_score") and row["Meta_score"] >= 70:  # critic score
        score += 1
    return score

unwatched_top_1000["predicted_score"] = unwatched_top_1000.apply(score_movie, axis=1)

# --- Sort recommendations ---
recommendations = unwatched_top_1000.sort_values(
    ["predicted_score", "IMDB_Rating", "No_of_votes"],
    ascending=[False, False, False]
).head(20)

# --- Display as a new table ---
st.write("These are the top recommended unwatched movies from IMDb Top 1000, based on your tastes:")
st.dataframe(
    recommendations[[
        "Title", "Year", "Genre", "IMDB_Rating", "Meta_score", 
        "Director", "No_of_votes", "predicted_score"
    ]],
    width="stretch",
    height=400
)
